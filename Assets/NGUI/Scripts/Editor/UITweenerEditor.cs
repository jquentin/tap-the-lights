...
public class UITweenerEditor : Editor
{
	...
	protected void DrawCommonProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Tweener"))
		{
			...
			GUILayout.BeginHorizontal();
			float delF = EditorGUILayout.FloatField("Start Delay Forward", tw.delayForward, GUILayout.Width(170f));
			GUILayout.Label("seconds");
			GUILayout.EndHorizontal();

			GUILayout.BeginHorizontal();
			float delB = EditorGUILayout.FloatField("Start Delay Backward", tw.delayBackward, GUILayout.Width(170f));
			GUILayout.Label("seconds");
			GUILayout.EndHorizontal();
			...
			if (GUI.changed)
			{
				...
				tw.tweenGroup = tg;
				tw.duration = dur;
				tw.delayForward = delF;
				tw.delayBackward = delB;
